        ...
        <div className="relative overflow-hidden rounded-2xl border-[3px] border-[#131C33] bg-[#4EC0E8]">
          ...
          {phase === 'cover' && (
            <Overlay lower>
              ...
              <h3 className="-mt-12 text-5xl sm:text-6xl font-black uppercase leading-none tracking-tight text-white drop-shadow-[0_3px_0_#131C33] text-center">
                Flappy<br />Picas
              </h3>
              {hud.best > 0 && (
...
const Overlay: React.FC<{ children: React.ReactNode; lower?: boolean }> = ({ children, lower }) => (
  <div
    className={`absolute inset-0 flex flex-col items-center px-6 text-center ${
      lower ? 'justify-end pb-[22%]' : 'justify-center'
    }`}
  >
    {children}
  </div>
